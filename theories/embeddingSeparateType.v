...
Definition arrow := <fun x => fun y => fun gamma => Pi (x gamma) (y gamma)>.
...
Inductive Typed0 : QTerm -> QTerm -> QTerm -> Prop :=
| ty_lambda : forall ctx A B s, Typed0 <`cons `ctx `A> <`weaken `B> s
                                -> Typed0 ctx <`arrow `A `B> <`lambda `s>
| ty_app : forall ctx A B s1 s2, Typed0 ctx <`arrow `A `B> s1 -> Typed0 ctx A s2
                                 -> Typed0 ctx B <`app `s1 `s2>
| ty_var : forall ctx T t, VarTyped ctx T t -> Typed0 ctx T t
| ty_true : forall ctx, Typed0 ctx Bool true
| ty_false : forall ctx, Typed0 ctx Bool false
| ty_if : forall ctx T cond t1 t2,
    Typed0 ctx Bool cond ->
    Typed0 ctx <`subLast `T `true> t1 ->
    Typed0 ctx <`subLast `T `false> t2 ->
    Typed0 ctx <`subLast `T `cond> <`ifexpr `cond `t1 `t2>
(* TODO: this should allow dependent types *)
(*
| ty_pi : forall ctx A B,
    Typed0 ctx <`U> A
    -> Typed0 <`cons `ctx `A> <`U> B -> Typed0 ctx <`U> <`pi `A `B>
| ty_Empty : forall ctx, Typed0 ctx <`U> Empty
*)
.
