...
Inductive Nf : QTerm -> Prop :=
| nf_ne : forall t, Ne t -> Nf t
| nf_lam : forall t s, Nf t -> Nf (lam s t)
| nf_pair : forall t1 t2, Nf t1 -> Nf t2 -> Nf (pair t1 t2)
| nf_pi1 : forall t, Nf t -> Nf (pi1 t)
| nf_pi2 : forall t, Nf t -> Nf (pi2 t)
with Ne : QTerm -> Prop :=
| ne_var : forall s i, Ne (var s i)
| ne_app : forall t1 t2, Ne t1 -> Nf t2 -> Ne (app t1 t2)
.
...
(*
Inductive definition of normal and neutral over the untyped terms
 *)

(* Very similar to the version from embeddingUniverses.v for proving consistency, except that
 the Empty and Bool cases include all neutral forms.
 The result of this is that ALL types include all neutral forms in their interperetations.
 in_ne ensures that this is true for the type of types.
 *)
...
| in_Bool :  In' Prev <Bool>
               (fun b => b = <fun x => proj1 x> \/ b = <fun x => proj2 x> \/ Ne b)
| in_Empty : In' Prev <Empty> Ne
...
| in_ne : forall T, Ne T -> In' Prev T Ne
.
...
Theorem In2_function : forall lvl T S1 S2, In2 lvl T S1 -> In2 lvl T S2 -> S1 = S2.
Proof.
  intros.
  inversion H.
  inversion H0.
  subst.
  rewrite (In_function _ _ _ _ H1 H5) in *.
  reflexivity.
Qed.
...
| in_cons : forall env ctx lvl val T s,
    InCtx env ctx
    -> In2 (S lvl) <`T `env> s (* is the successor here correct? *)
    -> s val
...
Theorem fundamental_lemma : forall ctx T lvl t env,
    Typed ctx lvl T t
    -> InCtx env ctx
    -> exists s, In2 (S lvl) <`T `env> s (* TODO: S lvl? In 0 is just empty. *)
    /\ s <`t `env>.
Proof.
  intros.
...
(* This could be extended to variables with indices other than 0. I'm not sure what I need. *)
Theorem Nf_lemma : forall t s, Nf <`t [`s] {var s 0}> -> Nf t.
Proof.
  intros.
  apply (@nf_lam _ s) in H.
  rewrite <- eta in H.
  assumption.
Qed.

Theorem InNf : forall lvl T S t, In lvl T S -> S t -> Nf t.
Proof.
  intros lvl.
  induction lvl.
  - intros. inversion H.
  - intros.
    ...
    (* Pi *)
    + intros.
      (* I think that this doesn't work, because we can't assume that there is any a such that
       (s a) holds. So I think that you really do need a new logical predicate which includes
       neutrals to make it work. *)
      (*specialize (H2 (var "TODO" 0)*)
      give_up.
    (* Bool*)
    + destruct H0; subst.
      * apply nf_lam.
    ...
    (* Type *)
    + destruct H0.
      specialize (IHlvl t x).
      give_up.
      (* I think I need to prove that if (In lvl T S) then Nf T separately? or as part
       of this induction? *)
    (* Lift *)
    + exact (IHlvl T S t H H0).
Qed.
